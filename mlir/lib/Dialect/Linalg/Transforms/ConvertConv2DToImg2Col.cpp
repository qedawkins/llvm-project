...
SmallVector<Value, 3> unrollIndex(
...
Value getConvolvedIndex(OpBuilder &b, Location loc,
...
rewriteInIm2Col(RewriterBase &rewriter, linalg::Conv2DNhwcHwcfOp convOp) {
  ...
  auto img2ColTensor = rewriter.create<linalg::GenericOp>(
      ...
      [&](OpBuilder &nestedBuilder, Location nestedLoc, ValueRange args) {
        ...
        auto mIndex = nestedBuilder.create<linalg::IndexOp>(loc, 1);
        auto kIndex = nestedBuilder.create<linalg::IndexOp>(loc, 2);

        // Recover the original iteration indices from the problem/input sizes.
        auto mIndices = unrollIndex(nestedBuilder, nestedLoc, mIndex,
        ...
        auto hIndex = getConvolvedIndex(nestedBuilder, nestedLoc,
                ohIndex, fhIndex, convOp.getStrides().getValues<int64_t>()[0]);
        auto wIndex = getConvolvedIndex(nestedBuilder, nestedLoc,
                owIndex, fwIndex, convOp.getStrides().getValues<int64_t>()[1]);
