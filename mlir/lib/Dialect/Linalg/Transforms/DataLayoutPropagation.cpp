...
static FailureOr<PackInfo> getPackingInfoFromOperand(
    AffineMap indexingMap, SmallVector<AffineMap> indexingMaps,
    SmallVector<utils::IteratorType> iterators, OpTy packOrUnPackOp) {
  ...
  // expression.
  OpBuilder b(packOrUnPackOp.getContext());
  for (auto i : packInfo.tiledDimsPos)
    for (auto map : indexingMaps)
      if (map.isFunctionOfDim(i) &&
          !map.getResultPosition(b.getAffineDimExpr(i)))
        return failure();
  ...
  // permutation.
  llvm::DenseSet<int64_t> permutedDomainDims;
  ...
  for (auto [index, dim] : llvm::enumerate(packOrUnPackOp.getOuterDimsPerm())) {
    ...
    if (auto dimExpr = permutedExpr.template dyn_cast<AffineDimExpr>()) {
      auto dimPos = dimExpr.getPosition();
      permutedOuterDims.push_back(dimPos);
      permutedDomainDims.insert(dimPos);
      continue;
    }
...
bubbleUpPackOpThroughGenericOp(RewriterBase &rewriter, tensor::PackOp packOp,
  ...
  auto packInfo =
      getPackingInfoFromOperand(genericOp.getMatchingIndexingMap(opOperand),
...
pushDownUnPackOpThroughGenericOp(RewriterBase &rewriter, GenericOp genericOp) {
  ...
  auto genericOutType =
      genericOp.getTiedOpResult(genericOp.getDpsInitOperand(0))
          .getType()
