...
LogicalResult scf::ForallOp::promoteIfSingleIteration(RewriterBase &rewriter) {
  ...
  // Promotion is only supported if all parallel updates are restricted to the
  // terminator body.
  if (!areAllUpdatesTerminal()) {
    return failure();
  }
...
Block::BlockArgListType ForallOp::getRegionSharedOuts() {
...
OpResult ForallOp::getTiedOpResult(BlockArgument sharedOut) {
  return getTiedOpResult(getTiedOpOperand(sharedOut));
}

/// Promotes the loop body of a scf::ForallOp to its containing block.
void mlir::scf::promote(RewriterBase &rewriter, scf::ForallOp forallOp) {
  assert(forallOp.areAllUpdatesTerminal() &&
         "forall promotion requires all updates in terminator.");
  OpBuilder::InsertionGuard g(rewriter);
...
// Checks if all in_parallel updates are present in the terminator.
bool ForallOp::areAllUpdatesTerminal(bool allowNonSlice) {
  Operation *terminator = getTerminator();
  for (auto sharedOut : getRegionOutArgs()) {
    for (Operation *user : sharedOut.getUsers()) {
      if (!isa<InParallelOpInterface>(user))
        continue;

      if (!allowNonSlice && !isa<tensor::ParallelInsertSliceOp>(user))
        return false;

      if (user->getParentOp() != terminator) {
        return false;
      }
    }
  }
...
struct ForallOpSingleOrZeroIterationDimsFolder
  ...
  LogicalResult matchAndRewrite(ForallOp op,
    ...
    if (newMixedLowerBounds.empty() && op.areAllUpdatesTerminal()) {
      promote(rewriter, op);
      return success();
    }
...
struct FoldTensorCastOfOutputIntoForallOp
  ...
  LogicalResult matchAndRewrite(scf::ForallOp forallOp,
                                PatternRewriter &rewriter) const final {
    if (!forallOp.areAllUpdatesTerminal(/*allowNonSlice=*/false)) {
      return rewriter.notifyMatchFailure(
          forallOp,
          "Unimplemented: casting non tensor.parallel_insert_slice update.");
    }
...
SmallVector<BlockArgument> InParallelOp::getDests() {
  ...
  for (auto &yieldingOp : getYieldingOps()) {
    auto inParallelOp = cast<InParallelOpInterface>(&yieldingOp);
